...
@ApiTags("Fulfillment Query")
@UseGuards(FulfillmentAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("fulfillments/query")
export class FulfillmentQueryController {
  #logger = new Logger(FulfillmentQueryController.name);

  constructor(private readonly service: FulfillmentQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all fulfillment with optional pagination" })
  @ApiResponse({ status: 200, type: FulfillmentsResponse })
  @ApiQuery({ name: "options", required: false, type: FulfillmentDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Fulfillment>    
  ): Promise<FulfillmentsResponse<Fulfillment>> {
    try {
     
      const fulfillments = await this.service.findAll(options);
      logger.info("Retrieving all fulfillment");
      return fulfillments;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find fulfillment by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter fulfillment', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: FulfillmentsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Fulfillment no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find fulfillments with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Fulfillment>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all fulfillments" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count fulfillments with conditions" })
  @ApiResponse({ status: 200, type: FulfillmentsResponse<Fulfillment> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one fulfillment with conditions" })
  ...
  @ApiOperation({ summary: "Find one fulfillment or return error" })
  @ApiResponse({ status: 200, type: FulfillmentResponse<Fulfillment> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the fulfillment to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<FulfillmentResponse<Fulfillment>> {
    try {
      const fulfillment = await this.service.findOne({ where: { id } });
      if (!fulfillment) {
        throw new NotFoundException(
