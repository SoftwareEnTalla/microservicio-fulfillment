...
export const AppDataSource = new DataSource({
  ...
  database: process.env.DB_NAME || "fulfillment-service",
...
async function checkPostgreSQLExtensions() {
  ...
  const fulfillment = await pool.connect();

  try {
    for (const ext of REQUIRED_EXTENSIONS) {
      const res = await fulfillment.query(
      ...
      if (res.rows.length === 0) {
        ...
        await fulfillment.query('CREATE EXTENSION IF NOT EXISTS "' + ext + '"');
      }
    }
  } finally {
    await fulfillment.release();
    await pool.end();
  }
}
